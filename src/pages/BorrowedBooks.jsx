...
import { createBarrowedBooks, deleteBarrowedBooks, getAllBooks, getBarrowedBooks, updateBarrowedBooks } from '../api/helper';
...
const BorrowedBooks = () => {
    ...
    const [deleteBorrowedBookId, setDeleteBorrowedBookId] = useState();
            ...
            render: (id, user) => (
                <Row style={{ gap: '20px' }}>
                    ...
                    <Popconfirm
                        ...
                        onCancel={() => setDeleteBorrowedBookId()}
                        okText="Yes"
                        cancelText="No"
                        open={deleteBorrowedBookId === id}
                    >
                        <span
                            style={{ cursor: 'pointer' }}
                            onClick={() => setDeleteBorrowedBookId(id)}
                        >
                            <DeleteOutlined />
                        </span>
                    </Popconfirm>
    ...
    const handleDeleteUser = async () => {
        try {
            setLoading(true);
            const { success } = await deleteBarrowedBooks(deleteBorrowedBookId);
            if (success) {
    ...
    const fetchBorrowedBooks = useCallback(async () => {
        if (userInfo && userInfo?.library_id) {
            ...
            try {
                setLoading(true);
                const { success, data = [] } = await getBarrowedBooks({ library_id });
                if (success) {
    ...
    const onFinish = async (values) => {
        ...
        try {
            if (mode === 'edit') {
                response = await updateBarrowedBooks({ ...state, ...payload })
            } else {
                response = await createBarrowedBooks(payload)
            }
